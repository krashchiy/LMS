    ...
    public class AccountController : CommonController
    {
        ...
        public string CreateNewUser(string fName, string lName, DateTime DOB, string SubjectAbbrev, string role)
        {
            ...
            else
            {
                ...
                string existingId = (from u in db.Users where u.FirstName == fName && u.LastName == lName && u.Dob == DOB select u.UserId).FirstOrDefault();
                ...
                {
                    userId = (from u in db.Users select u).OrderByDescending(x => x.UserId).Select(u => u.UserId).FirstOrDefault();
                    int numeric = Convert.ToInt32(userId.Substring(1));
                    numeric++;
                    userId = userId.Remove(userId.Length - numeric.ToString().Length) + numeric.ToString();
            ...
            int deptId = (from d in db.Departments where d.Abbrev == SubjectAbbrev select d.DeptId).FirstOrDefault();

            if (!userExists && deptId > 0)
            {
